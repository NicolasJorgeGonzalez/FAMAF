...
interp_resize x imagen d w h = imagen d (fromIntegral x V.* w) (fromIntegral x V.* h) 

--COMPLETAR (EJERCICIO 2-a)
interp_basica :: Bool -> ImagenFlotante
interp_basica a | (a == True) = formaF(blue)
...
interp (Resize n1 (dib))           = interp_resize n1 (interp (dib))