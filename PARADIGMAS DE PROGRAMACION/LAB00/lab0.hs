...
--
duplica' :: [Int] -> [Int]
duplica' xs = generalMap xs ( *2)

par :: Int -> Bool
par x = ((x `mod` 2) == 0)

esPar :: [Int] -> [Bool]
esPar xs = morePolGeneralMap xs par
...
moreGeneralFilter :: [a] -> (a -> Bool) -> [a]
moreGeneralFilter [] f     = []
moreGeneralFilter (x:xs) f | f x = x : moreGeneralFilter xs f
                           | otherwise = moreGeneralFilter xs f

soloPares' :: [Int] -> [Int]
soloPares' xs = moreGeneralFilter xs par
...
generalFold [] f n     = n
generalFold (x:xs) f n = generalFold xs f (f n x)
...
area (Rectangulo base alto)   = base * alto
area (Punto)                  = error "no se puede calcular el area de un punto"
...
evaluar :: Expr -> Int
evaluar (Numero n) = n
evaluar (Suma e n) = evaluar e + n
evaluar (Producto e n) = evaluar e * n
evaluar (Resta e n) = evaluar e - n
evaluar (Division e n) = evaluar e `div` n
...
data BinTree a = Nodo (BinTree a) a (BinTree a)
                 deriving (Eq, Show)
